...
import art.meiye.venus.service.vo.account.EmailBindRequest;
 ...
 * @date 2020-08-10
 */
public class EmailBindValidator implements ConstraintValidator<EmailBindValidation, EmailBindRequest> {
    @Autowired
    UserIdentityBizHandle userIdentityBizHandle;

    @Override
    public boolean isValid(EmailBindRequest request, ConstraintValidatorContext context) {
        ...
        if (userIdentityBizHandle.isEmailExist(request.getEmail())) {
            ErrorPayload.addErrorToHibernateContextFinal(hibernateContext, "email", ErrorMapping.ACCOUNT_EMAIL_BIND_EXIST);
            return false;
        }
        // 6. 校验邮箱和验证码是否匹配
