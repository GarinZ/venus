...
import art.meiye.venus.service.vo.auth.PasswordResetRequest;
 ...
 * @date 2020-08-10
 */
public class PasswordResetValidator implements ConstraintValidator<PasswordResetValidation, PasswordResetRequest> {
    @Autowired
    ...
    public boolean isValid(PasswordResetRequest request, ConstraintValidatorContext context) {
        ...
        // 5. 验证码错误阈值校验
        if (userIdentityBizHandle.isForbiddenVerificationValidate(request.getEmail(), VerificationEnum.PASSWORD_RESET)) {
        ...
        if (!userIdentityBizHandle.isEmailAndVerificationCodeMatches(request.getEmail(), request.getVerificationCode())) {
            // 验证码错误计数器增加
            userIdentityBizHandle.processVerificationFail(request.getEmail(), VerificationEnum.PASSWORD_RESET);
            ErrorPayload.addErrorToHibernateContextFinal(hibernateContext, "verificationCode", ErrorMapping.EMAIL_VERIFICATION_NOT_MATCH);
            return false;
        }
        // 验证码错误计数器清空
        userIdentityBizHandle.processVerificationSuccess(request.getEmail(), VerificationEnum.PASSWORD_RESET);
        return true;
    }
}
